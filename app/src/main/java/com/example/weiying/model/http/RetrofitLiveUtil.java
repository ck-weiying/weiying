package com.example.weiying.model.http;
 ...
 */
public class RetrofitLiveUtil {
    private Retrofit retrofit;
    private static RetrofitLiveUtil retrofitUtil;

    private RetrofitLiveUtil() { }
    private RetrofitLiveUtil(String baseUrl) {
        //第三方的日志拦截器
...
//                .writeTimeout(50, TimeUnit.SECONDS)
    ...
    //可指定baseUrl
    public static RetrofitLiveUtil getInstance(String baseUrl){
        if (retrofitUtil==null){
            synchronized (RetrofitLiveUtil.class){
                if (null==retrofitUtil){
                    retrofitUtil = new RetrofitLiveUtil(baseUrl);
                }
            }
        }
        return retrofitUtil;
    ...
    //默认的baseUrl
    public static RetrofitLiveUtil getInstance(){
        if (null == retrofitUtil){
            return  getInstance(Constants.LIVE_BASE_URL);
        }
        return retrofitUtil;
    }
    //获得Retrofit
